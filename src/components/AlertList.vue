        ...
        <tbody>
          <tr v-for="alert in patientAlerts" :key="alert.patientID">
            ...
            <td>
              ...
              <table v-else>
                <tr v-for="item in alert.alertItemList" :key="item.serialNo">
                  <router-link :to="{name:'PatientInfo', query:{patientID:alert.patientID, alertSerialNo:item.serialNo, interveneAll:false}, params:{isOptional:true}}">干预</router-link>|
            ...
            <td>
              <a v-if="type===0&&$dict.manageStatus[alert.manageItem.manageStatus]!=='管理中'">不可操作</a>
              <div v-else>
                <router-link :to="{name:'PatientInfo', query:{patientID:alert.patientID, interveneAll:true}, params:{patientAlertList:alert.alertItemList}, params:{isOptional:true}}">全部干预</router-link>|
